...
class DGA(nn.Module):
    def __init__(self, in_feats, n_hidden, num_nodes, n_classes, n_etypes, p=0.3, n_head=1,
        ...
        # 输出层
        all_layers = []
        all_layers.append(nn.Linear(n_hidden, n_hidden // 2))
        all_layers.append(nn.ReLU())
    ...
    def forward(self, blocks, x):
        ...
        x = x + 0.1 * F.relu(risk_feature)  # 使用ReLU确保风险为非负
...
class pl_DGA(LightningModule):
    ...
    def validation_epoch_end(self, outs):
        ...
        if self.trainer.sanity_checking:
            ...
            # No-group ablation: dynamic grouping feedback is disabled
            ...
            # 严格协议：训练期间只报告训练集和验证集指标
            # 测试集仅在训练完成并加载最佳 checkpoint 后评估一次
