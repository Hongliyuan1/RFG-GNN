...
class DGA(nn.Module):
    """Graph-free MLP baseline using the same encoder and classifier."""
    ...
    def forward(self, blocks, x):
        ...
        # Use destination-node features only. This prevents the MLP and its
        # BatchNorm statistics from depending on sampled graph neighbors.
        x_dst = x[: block.number_of_dst_nodes()]
        h = self.emb_layer(x_dst)
